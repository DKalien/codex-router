...
- zstd 请求使用异步解压，并在进入原生解码器前检查帧头声明尺寸；保留解压输出上限。
- 第三方普通和压缩请求会将缺少调用 ID 的具名 `codex_app` 工具结果恢复为可读历史，
  保留结果内容；官方请求和其他工具结果保持原样。
- Windows 非交互后台 PowerShell 调用隐藏控制台窗口，覆盖凭据 ACL 和服务管理。

- WLB 全局路由按官方选择器的 `gpt-*` 模型名直接转发，不再要求本地逐个登记；
  新模型是否可用由上游决定，上游错误不会触发官方回退。
